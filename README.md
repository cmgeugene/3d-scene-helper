...
I2V 시작 프레임을 빠르게 구성하는 로컬 우선 React/TypeScript/Vite 3D 씬 디렉팅 도구입니다. 기본 바닥·1.7m 마네킹·OutputCamera가 있는 starter scene에서 primitive, 코너형 방 세트와 리깅 GLB 캐릭터를 배치하고 카메라·조명·구도·motion guide를 설정할 수 있습니다. 완성한 장면은 clean/reference PNG 또는 scene JSON으로 내보내거나, Local Companion의 대화형 Scene Assistant와 레퍼런스·이미지 생성 워크플로로 이어갈 수 있습니다.
...
연결 후 Scene Assistant 입력창에 장면 설명이나 질문을 보내면 현재 장면 스냅샷과 OutputCamera 기준 `LayoutSpec`을 함께 전달해 Codex task를 시작하거나 저장된 task를 재개합니다. WebGL 캡처가 가능하면 현재 3D 레이아웃 렌더를 대화의 첫 첨부로 보내고, 장면과 카메라가 바뀌지 않은 후속 대화에서는 마지막 렌더를 재사용합니다. 마네킹의 월드 회전, 카메라 상대 방향과 화면상 방향은 서로 구분해 전달하며, 캡처가 실패해도 구조화된 장면 정보만으로 대화를 계속합니다. 응답은 이벤트 스트림으로 표시되며 진행 중에는 중단할 수 있습니다. task ID와 제한된 대화 요약은 프로젝트의 `conversations.json`에 저장되고, 프로젝트를 다시 열면 저장 task 재개 또는 새 task 시작을 명시적으로 선택합니다. 연결이 끊기면 0.5초, 1초, 2초 간격으로 최대 3회 상태를 다시 조회합니다. 이 과정에서 turn을 자동 재전송하지 않으며, 복구 후 다음 사용자 동작이 저장 thread를 이어 갑니다.

화면 아래 References 트레이에서는 PNG, JPEG, WebP 이미지를 `Layout`, `Background`, `Character`, `Style` 역할로 가져올 수 있습니다. 파일은 25MB 이하만 허용하며 실제 이미지 시그니처를 검사합니다. 원본은 프로젝트의 `assets/references/`에 복사되고, 안정적인 ID·SHA-256 해시·크기·역할은 프로젝트 루트의 `references.json`에 기록됩니다. 트레이에는 썸네일과 메타데이터가 표시되며 생성에 사용할 레퍼런스를 선택할 수 있습니다. 각 카드의 `설정`에서 캐릭터 레퍼런스를 장면의 마네킹에 연결하고, 쉼표로 구분한 사용 범위와 제외 범위를 저장할 수 있습니다. 카드별 삭제와 선택 항목 일괄 삭제, 프로젝트 레퍼런스 전체 삭제도 확인 후 원본과 manifest에 함께 반영합니다.
...
WebGL 뷰포트가 준비된 상태에서는 Scene Assistant에 연출 지시를 입력하고 `이미지 생성`을 누를 수 있습니다. 앱이 현재 OutputCamera를 reference PNG로 캡처해 fresh/edit 모두 첫 번째 첨부로 고정합니다. edit에서는 기존 키프레임이 두 번째이며 선택한 레퍼런스는 그 뒤에 역할 순서대로 붙습니다. 기본 OAuth 경로에서는 실제 Codex `$imagegen` 스킬이 이미지 역할·권위, primary request, style/integration과 strict invariants를 포함한 최종 전달 prompt를 먼저 작성합니다. 선택한 Responses 모델은 그 prompt 자체와 같은 이미지 입력을 받아 선택한 quality로 `image_generation`을 실행합니다. Companion이 attachment contract v2의 canonical descriptor를 만들고, prompt compiler는 구조화된 binding과 사람이 읽는 `Image 1…N` 역할 문구가 descriptor와 일치하는지 검증합니다. Image 1 레이아웃은 카메라·크롭·원근·배치·크기·포즈·깊이·가림의 최상위 권위이며, 기존 키프레임은 외형 권위만 가집니다. 역할 재분류, 권위 약화, passive item 외의 도구나 승인 요청은 fail-closed 처리합니다. 마지막으로 정상 완료된 Companion 대화 교환은 revision이 있는 generation intent로 자동 승격되며, 실패·중단 turn이나 원시 transcript 전체는 전달하지 않습니다. 진행 상태와 완료 이미지는 패널에 표시됩니다. 구도 캡처는 `assets/scene-renders/`, 생성 결과는 `assets/generations/`에 복사되며 요청·첨부·해시·attachment contract/binding·provider·모델·quality·reasoning·원본 prompt·imagegen 스킬 최종 전달 prompt(`generationSpec`, 호환 필드명)·도구 revised prompt·반영된 대화 의도는 `generations.json`에 구분해 기록됩니다. 실제 생성은 ChatGPT/Codex 사용량을 소비하므로 자동 테스트에서는 모의 OAuth/Responses 또는 App Server 이벤트만 사용합니다.
...
새 generation record에는 생성 당시의 전체 SceneDocument와 서버가 확인한 레퍼런스 메타데이터가 불변 스냅샷으로 저장됩니다. 부모 generation ID, 계보상의 버전 번호, 피드백과 `fresh/edit` 생성 방식도 함께 기록합니다. 기본 생성은 3D 레이아웃 1장과 레퍼런스 최대 4장을 사용합니다. 완료 결과에서 `이 결과를 기반으로 보정`을 선택하면 현재 3D 레이아웃을 Image 1, 기존 키프레임을 Image 2로 고정하고 레퍼런스 최대 3장을 뒤에 붙이는 `edit` 자식 generation을 만듭니다. 같은 보정 모드에서 다시 생성하면 직전 파생본이 아니라 선택한 원본 generation에서 형제 버전을 만듭니다. 스냅샷 필드가 없던 기존 기록은 계속 열 수 있지만 `sceneSnapshot: null`로 표시되어 향후 3D 복원이 제한됩니다.

생성 전에 앱은 현재 장면에서 `LayoutSpec v2`를 결정적으로 계산합니다. 이 명세에는 출력 카메라, 각 오브젝트의 정규화 화면 바운드와 점유율, 카메라 깊이, 전경·중경·배경 분류, 마네킹 포즈와 카메라 상대 방향, 연결된 외형 레퍼런스, 잠재 가림 관계, 그룹 ID, 프록시 표시, 최종 표면 의도, 구조화된 포함 관계, 평면 거울의 화면 경계·월드 법선·필수 반사 대상이 포함됩니다. Scene Assistant의 `3D → 키프레임 변환 계약`에서 요약을 확인할 수 있습니다. 생성 prompt는 3D 레이아웃이 카메라·크롭·배치·포즈·깊이·가림의 기준이고 프록시 색·재질·외형은 역할별 레퍼런스와 사용자 설명으로 교체된다는 규칙을 명시합니다. 특히 프록시 opacity는 편집용 표시일 뿐 최종 투명도를 결정하지 않으며, 최종 표면 타입과 내부 오브젝트 가시성만 이미지 의도로 사용합니다. 거울 반사상은 지정된 mirror bounds 안에만 만들고 같은 인물이나 소품을 장면의 별도 실물로 복제하지 않도록 지시합니다. 사용된 전체 LayoutSpec은 해당 generation record에 함께 저장됩니다.

씬 목록에서 오브젝트를 선택하면 우측 `씬` 속성에서 이름을 바꾸고 `이미지 생성 의미`를 입력할 수 있습니다. 각 행의 잠금 버튼은 뷰포트 클릭 선택만 막습니다. 잠긴 오브젝트도 씬 목록에서는 계속 선택할 수 있고 Inspector와 transform gizmo를 그대로 사용할 수 있으며, 앞의 잠긴 오브젝트를 통과해 뒤의 잠기지 않은 오브젝트를 클릭할 수 있습니다. Ctrl/⌘ 또는 Shift를 누른 채 여러 행을 선택하면 그룹화할 수 있습니다. 그룹 선택 후 XYZ 이동량을 적용하면 모든 멤버가 같은 월드 delta로 한 번에 이동하며 undo/redo도 그룹 이동 전체를 하나의 작업으로 다룹니다. 그룹 회전·스케일·중첩 그룹은 아직 지원하지 않습니다. 이름은 아웃라이너와 캐릭터 레퍼런스 연결 대상에 즉시 반영됩니다. `실제 의미`에는 `빨간 원형 포차 테이블`, `아웃포커스된 전봇대`처럼 최종 이미지의 사물명을, `생성 메모`에는 재질·초점·교체 규칙처럼 해당 오브젝트에만 적용할 지시를 적습니다. `프록시 표시와 최종 표면`에서는 내부 배치를 확인하기 위한 뷰포트 불투명도와 최종 이미지의 opaque/transparent/translucent 재질 의도를 별도로 편집합니다. plane을 `평면 거울`로 지정하면 기존 오브젝트 중 거울에 반드시 비쳐야 할 대상을 체크할 수 있고, 뷰포트와 clean PNG가 같은 반사 평면을 사용합니다. 체크 목록은 물리적으로 다른 사물을 숨기는 필터가 아니라 이미지 생성 모델에 전달할 필수 반사 대상 계약입니다. `내부 오브젝트 관계`에서는 다른 오브젝트를 내부 대상으로 지정하고 외부에서 가림, 열린 부분, 투명 표면 또는 컷어웨이를 통한 가시성을 선택합니다. 이 값은 SceneDocument에 저장되고 생성 시 LayoutSpec으로 구조화해 전달됩니다.

현재 장면 형식은 `SceneDocument v4`입니다. v1~v3 파일과 generation snapshot은 불변 migration으로 v4에 복원되며, 오브젝트별 뷰포트 선택 잠금, 프록시 opacity와 최종 표면 의도, 그룹·구조화된 공간 관계 기본값을 추가합니다. 현재 UI에서는 선택 잠금, translate-only 그룹, 프록시 표시, 최종 표면 의도, containment와 평면 거울 reflection을 편집할 수 있습니다. mirror는 plane만 지원하며 곡면·재귀·거울 속 거울은 지원하지 않습니다.

오브젝트 추가 패널에는 번들 `Meshy Idle` 캐릭터가 포함되어 있으며, Companion 연결 시 Rodin/Meshy 등에서 만든 스킨된 GLB 2.0 캐릭터를 프로젝트 자산으로 가져올 수 있습니다. 브라우저와 Companion이 파일 형식과 스킨 정보를 검증하고, 크기·중심·전방 방향·본·애니메이션·IK 체인을 분석해 프로젝트의 `models/`와 `rigged-character-assets.json`에 저장합니다. 가져온 캐릭터는 높이 조절, 포함된 첫 애니메이션의 시간 선택·Idle 미리보기, 매핑 가능한 경우 양손·양발 IK 편집을 지원하며 장면 저장과 이미지 레이아웃 분석에 포함됩니다.
...
Companion은 `127.0.0.1`에만 바인딩하고 실행 시 세션 토큰을 생성합니다. 일반 Codex 설치 경로나 로그인 캐시를 직접 읽지 않으며, 프로젝트의 `assets/` 내부 상대 경로만 이미지 입력으로 전달합니다. 현재 API는 런타임 상태와 imagegen capability, thread 시작·재개, turn 시작·취소, 레퍼런스 가져오기·목록·설정 갱신·삭제·콘텐츠 조회, 리깅 캐릭터 가져오기·목록·콘텐츠 조회, 구도 렌더 업로드, 생성 시작·목록·결과 조회와 SSE 이벤트를 제공합니다.

## 3분 starter scene 흐름

1. 데스크톱 Chromium에서 앱을 열고 기본 scene의 바닥과 마네킹을 확인합니다.
2. 필요하면 cube, sphere, cylinder, plane, mannequin, 리깅 GLB 캐릭터 또는 코너형 방 세트를 추가하고 inspector나 transform gizmo로 배치합니다. 방 세트는 바닥·뒤쪽 벽·왼쪽 벽으로 구성되어 천장과 카메라 쪽 두 면이 열려 있습니다.
...
- 모바일 touch 편집, cloud 저장/협업, 일반 목적의 임의 메시·재질·텍스처 가져오기, animation timeline과 physics는 현재 지원하지 않습니다. 3D asset import는 스킨된 캐릭터 GLB 워크플로로 제한됩니다.
