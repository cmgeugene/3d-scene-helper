...
### P0. 키프레임 작업 공간과 생성 이력

목표는 생성 결과에 피드백이 들어왔을 때 결과와 당시 3D 근거를 잃지 않고 다시 작업하는
것이다.

- `3D 씬`과 `키프레임` 작업 모드 제공
- 전체 generation 목록과 상태, 버전, 부모·자식 관계 표시
- 선택한 결과 이미지와 생성 당시 레이아웃 렌더 나란히 보기
- 생성 당시 지시, 피드백, 레퍼런스와 `LayoutSpec` 상세 보기
- 결과 이미지뿐 아니라 저장된 레이아웃 렌더를 읽는 Companion 콘텐츠 API
- 기존 보정 진입점을 최신 결과 카드 하나가 아니라 선택한 generation에 연결
...
다음 개발 세션은 **P0 키프레임 작업 공간과 생성 이력**으로 시작한다. 첫 수직 슬라이스는
generation 선택 UI, 선택 결과 조회, 생성 당시 레이아웃 렌더 콘텐츠 API와 두 이미지 비교
보기까지다. 3D 스냅샷을 실제 편집기에 적용하는 동작은 이 비교 화면을 검증한 뒤 P1에서
추가한다.
